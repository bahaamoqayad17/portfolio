...
export default function AboutPage() {
  ...
  const handleSaveAbout = async (updatedAbout: AboutInfo) => {
    try {
      setLoading(true);
      const response = await fetch('/api/about', {
        method: 'PUT',
      ...
      if (response.ok) {
        ...
        setAboutInfo(data);
        setEditing(false);
        setAlert({ message: 'About information updated successfully!', severity: 'success' });
      } else {
        setAlert({ message: 'Failed to update about information. Please try again.', severity: 'error' });
      }
    } catch (error) {
    ...
    <Box>
      ...
      {editing ? (
                  ...
                  {aboutInfo.categories.map((category) => (
                    <Grid item xs={12} sm={6} key={category.id}>
                      <Paper sx={{ 
                        ...
                        >
                          {category.icon === 'web' && <DesignServicesIcon fontSize="large" />}
                          {category.icon === 'server' && <StorageIcon fontSize="large" />}
                          {category.icon === 'code' && <CodeIcon fontSize="large" />}
                        </Box>
        ...
        <Paper sx={{ p: 4, textAlign: 'center' }}>
          <Typography variant="h6" sx={{ mb: 2 }}>No about information available</Typography>
          <Button
            variant="contained"
            startIcon={<EditIcon />}
            onClick={handleEditToggle}
