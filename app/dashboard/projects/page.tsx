...
export default function ProjectsPage() {
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (projectIdFromParams) {
            const project = data.find((p: Project) => p.id === projectIdFromParams);
            if (project) {
  ...
  const handleSaveProject = async (project: Project) => {
    try {
      ...
      if (response.ok) {
        ...
        setAlert({ message: 'Failed to save project. Please try again.', severity: 'error' });
      }
    } catch (error) {
  ...
  const handleDeleteProject = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setAlert({ message: 'Failed to delete project. Please try again.', severity: 'error' });
      }
    } catch (error) {
  ...
  return (
    <Box>
              ...
              <Grid item xs={12} sm={6} md={4} key={project.id}>
                ...
                >
                  <Card sx={{ 
                    ...
                    <Box sx={{ 
                      background: `url(https://via.placeholder.com/400x200?text=${encodeURIComponent(project.title)})`,
      ...
      >
        <MenuItem onClick={() => currentProject && handleEditProject(currentProject)}>
          <EditIcon fontSize="small" />
          <Typography variant="body2">Edit Project</Typography>
        </MenuItem>
        <MenuItem onClick={() => currentProject && handleDeleteConfirm(currentProject)}>
          <DeleteIcon fontSize="small" color="error" />
          <Typography variant="body2" color="error.main">Delete Project</Typography>
        </MenuItem>
      </Menu>
    </Box>
  );
