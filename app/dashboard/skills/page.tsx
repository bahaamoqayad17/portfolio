...
export default function SkillsPage() {
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (skillIdFromParams) {
            const skill = data.find((s: Skill) => s.id === skillIdFromParams);
            if (skill) {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (skill.id) {
          ...
          setAlert({ message: 'Skill updated successfully!', severity: 'success' });
        } else {
          // Add new skill
          setSkills(prev => [...prev, savedSkill]);
          setAlert({ message: 'Skill created successfully!', severity: 'success' });
        }
        ...
        setAlert({ message: 'Failed to save skill. Please try again.', severity: 'error' });
      }
    } catch (error) {
  ...
  const handleDeleteSkill = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setAlert({ message: 'Skill deleted successfully!', severity: 'success' });
      } else {
        setAlert({ message: 'Failed to delete skill. Please try again.', severity: 'error' });
      }
    } catch (error) {
  ...
  const skillsByCategory = filteredSkills.reduce((acc: Record<string, Skill[]>, skill) => {
  ...
  return (
    <Box>
              ...
              <Card sx={{ 
                ...
                <CardActions sx={{ px: 3, pb: 2, pt: 0 }}>
                  <Button 
                    ...
                    size="small"
                    onClick={() => {
                      setCurrentSkill({ id: '', name: '', level: 80, description: '', category });
                      setOpenForm(true);
                    }}
                  >
                    Add to {category}
      ...
      >
        ...
        <MenuItem onClick={() => currentSkill && handleDeleteConfirm(currentSkill)}>
          <DeleteIcon fontSize="small" color="error" />
          <Typography variant="body2" color="error.main">Delete Skill</Typography>
        </MenuItem>
      </Menu>
    </Box>
  );
